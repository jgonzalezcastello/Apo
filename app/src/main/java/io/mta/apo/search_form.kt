...
class search_form : AppCompatActivity() {
    ...
    fun init(): Unit {
        ...
        pill_name = findViewById<EditText>(R.id.search_form_et_pill_name)
        pill_imprint = findViewById<EditText>(R.id.search_form_et_pill_imprint)
        pill_color = findViewById<EditText>(R.id.search_form_et_pill_color)

        submit_form_Button  = findViewById<ImageButton>(R.id.search_form_ib_submit_form)
        dev_log = findViewById<TextView>(R.id.search_form_tv_response_view)

        submit_form_Button.setOnClickListener({submit_click_listener()})
    }

    fun submit_click_listener(){
        dev_log.text = pill_request_server.queryServerPillOption(pill_name.text.toString(),pill_imprint.text.toString(),pill_color.text.toString())
    }
}
